...
async def catch_direct_session(session_id: str):
    if session_id.startswith("session_"):
        history = CHAT_SESSIONS.get(session_id, [])
        return {"session_id": session_id, "history": history, "messages": history}
    raise HTTPException(status_code=404, detail="Resource not found")import os
import shutil
import threading
from datetime import datetime
from contextlib import asynccontextmanager
...
async def chat_endpoint(request: QueryRequest):
    ...
    try:
        ...
        return JSONResponse(
            status_code=200,
            content={"query": request.query, "answer": "This information is not available in the college records.", "session_id": request.session_id}
        )
